...
inline System::Void kurs::MyForm::MyForm_Load(System::Object^ sender, System::EventArgs^ e) {
    F1->id = 0;
    F1->name = "Полиэф";
    F1->description = "«Полиэф» — крупнейший в России производитель полиэтилентерефталата (ПЭТ) и терефталевой кислоты. \nПредприятие находится в г. Благовещенске (Республика Башкортостан), входит в состав Дирекции пластиков, эластомеров, органического синтеза СИБУРа.";
    F1->address = "453434, Российская Федерация, Республика Башкортостан, г. Благовещенск, ул. Социалистическая, д. 71";
    F1->email = "inbox@polief.sibur.ru";
    F1->reputation = "10";
    F1->staff_numbers = "743";
   
    F2->id = 1;
    F2->name = "Уфаоргсинтез";
    F2->description = "«Уфаоргсинтез» — производитель продукции органического синтеза, одно из четырёх промышленных градообразующих предприятий по переработке нефти в городе Уфе[1][2][3]. Принадлежит акционерной нефтяной компании «Башнефть».";
    F2->address = "450037, Российская Федерация, Республика Башкортостан, г. Уфа.";
...
System::Void kurs::MyForm::button_save_Click(System::Object^ sender, System::EventArgs^ e)
{
    if (tabControl->SelectedTab->Name == "tabPage_p") {
        ...
        MessageBox::Show("Список товаров для завода «Полиэф» сохранён!", "Успешно!", MessageBoxButtons::OK, MessageBoxIcon::Information);
    }
    else if (tabControl->SelectedTab->Name == "tabPage_uos") {
        save_data_into_file(goods2, n2, PATH_UOS);
        MessageBox::Show("Список товаров для завода «Уфаоргсинтез» сохранён!", "Успешно!", MessageBoxButtons::OK, MessageBoxIcon::Information);
    }
}
...
System::Void kurs::MyForm::infoMenuItem_Click(System::Object^ sender, System::EventArgs^ e)
{
    if (tabControl->SelectedTab->Name == "tabPage_p") {
        ...
        MessageBox::Show(gcnew System::String(info.c_str()), "Завод «Полиэф»", MessageBoxButtons::OK);
    }
    else if (tabControl->SelectedTab->Name == "tabPage_uos") {
        std::string info = F2->description + "\nКоличество работников: " + F2->staff_numbers + "\nРейтинг: " + F2->reputation + "\nEmail: " + F2->email + "\nАдрес: " + F2->address;
        MessageBox::Show(gcnew System::String(info.c_str()), "Завод «Уфаоргсинтез»", MessageBoxButtons::OK);
    }
}
...
inline System::Void kurs::MyForm::dataGridView_MouseClick(System::Object^ sender, System::Windows::Forms::MouseEventArgs^ e)
{
    ...
    System::Windows::Forms::MenuItem^ infoMenuItem = gcnew MenuItem("Информацция о заводе");
...
System::Void kurs::MyForm::заводПолиэфToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e)
{
    if (!goods1 || !goods2) {
        MessageBox::Show("Сначала загрузите данные!", "Внимание!", MessageBoxButtons::OK, MessageBoxIcon::Warning);
        return;
    }
    MessageBox::Show("Общее количество товаров на заводе «Полиэф»: " + gcnew System::String(summary_amount_of_items(goods1, n1).ToString()), "Результат", MessageBoxButtons::OK);
}
...
System::Void kurs::MyForm::заводУфаоргсинтезToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e)
{
    ...
    MessageBox::Show("Общее количество товаров на заводе «Уфаоргсинтез»: " + gcnew System::String(summary_amount_of_items(goods2, n2).ToString()), "Результат", MessageBoxButtons::OK);
}
...
System::Void kurs::MyForm::заводПолиэфToolStripMenuItem1_Click(System::Object^ sender, System::EventArgs^ e)
{
    ...
    if (!n1) {
        MessageBox::Show("Нет товаров на заводе!", "Внимание!", MessageBoxButtons::OK, MessageBoxIcon::Warning);
        return;
    }
    double max = 0;
    int index;
